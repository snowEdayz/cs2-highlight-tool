<template>
  <div class="sub-page">
    <n-card
      ...
      content-class="wanmei-sub-card-content"
    >
      <div class="panel-head">
        <div class="panel-head-row">
          <n-space align="center" :size="8" class="panel-head-left">
            ...
            <span class="panel-title">{{ t("main.import.sub_wanmei_title") }}</span>
          </n-space>
          <div class="header-actions">
            <n-text depth="3" class="queue-text">
              {{ t("main.import.wanmei_import_queue", { running: runningImportCount, queued: queuedImportCount }) }}
            </n-text>
            <n-button
              size="small"
              :loading="loading"
            ...
            >
              {{ t("main.import.wanmei_refresh") }}
            </n-button>
          </div>
        </div>
      </div>
      <div class="wanmei-content">
        <div class="manual-import-box">
          <n-space vertical :size="6">
            <n-text depth="3">{{ t("main.import.wanmei_manual_hint") }}</n-text>
            <div class="manual-input-row">
              <n-input
                ...
                :placeholder="t('main.import.wanmei_manual_placeholder')"
              />
              <n-button
                size="small"
              ...
              >
                {{ t("main.import.wanmei_manual_import_action") }}
              </n-button>
            </div>
          </n-space>
        </div>
...
import { useWanmeiImport } from "@/features/import/composables/useWanmeiImport";
...
const {
  ...
  enqueueMatchImport,
  submitManualMatches,
} = useWanmeiImport(onDemosSelected);
...
const columns = computed<DataTableColumn[]>(() => [
  ...
  {
    title: () => t("main.import.wanmei_col_time"),
    key: "end_time",
    width: "18%",
    ellipsis: true,
  },
  ...
  {
    title: () => t("main.import.wanmei_col_kda"),
    key: "kda",
    width: "11%",
    render: (row: any) => `${row.kill}/${row.death}/${row.assist}`,
  },
  {
    title: () => t("main.import.wanmei_col_remark"),
    key: "remark",
    width: "11%",
    render: (row: any) => formatWanmeiRemark(row),
  },
  {
    title: () => t("main.import.wanmei_col_rating"),
    key: "rating",
    width: "8%",
    render: (row: any) => formatWanmeiRating(row.rating),
  },
  {
    title: () => t("main.import.wanmei_col_action"),
    key: "actions",
    width: "24%",
    fixed: "right",
    align: "center",
    render: (row: any) => {
      const matchID = row.download_match_id;
      const taskState = resolveTaskState(matchID);
      if (taskState === "running") {
        const percent = resolveDownloadPercent(matchID);
        return h("span", { class: "wanmei-import-loading" }, [
          h(NSpin, {
            size: 14,
            stroke: "#2f9462",
          }),
          typeof percent === "number"
            ? h("span", { class: "wanmei-import-percent" }, `${percent}%`)
            : null,
        ]);
      }
      if (taskState === "queued") {
        return h(
          ...
          {
            default: () => t("main.import.wanmei_import_queued"),
          },
        );
      }
      return h(
        ...
        { default: () => t("main.import.wanmei_import_action") },
      );
    },
  },
]);
...
.sub-card :deep(.wanmei-sub-card-content) {
  flex: 1;
  min-height: 0;
  overflow: hidden;
}
